...
const PointLayout = () => {
    ...
    const pointItemOnClick = (id: string, isCheckbox: boolean) => {
        ...
        if (isShiftKeydown && prevClickedUserId.current) {
            const firstIndex = nonUndefinedData.findIndex(
                (student) => student.id === prevClickedUserId.current,
            );
            let secondIndex: number = -1;
            if (prevMultiSelectedUserId.current && prevMultiSelectedUserId.current !== null)
                secondIndex = nonUndefinedData.findIndex(
                    (student) => student.id === prevMultiSelectedUserId.current,
                );
            const prevSlicedStudents = nonUndefinedData.slice(
            ...
            const slicedStudents = nonUndefinedData.slice(
                Math.min(firstIndex, secondIndex),
                Math.max(firstIndex, secondIndex) + 1,
            );
            setSUI((prev) => ({
